...
/**
 * A better and more capable version of the {@link YamlConfiguration} from Bukkit.
 *
 * Benefits:
 * - Easy creation of infinite custom config files.
 * - Automatically updates the config on the disk with the latest version.
 * - Increased safety.
 *
 * @author SpigotCourse
 */
public class FileManager extends YamlConfiguration {
  ...
  /**
   * Permanently removes the configuration file from your disk.
   *
   * You will still be able to use the config as it is loaded in the plugin
   */
  ...
  private File extract(String path) {
    final JavaPlugin i = OmegaVision.getInstance();
  ...
  private File createFileAndDirectory(String path) {
    ...
    final File datafolder = OmegaVision.getInstance().getDataFolder();
